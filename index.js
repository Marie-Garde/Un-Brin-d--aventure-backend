...
app.get("/api/client/name/:name?/firstname/:firstname?/city/:city?/email/:email?", (req, res) => {
...
app.post("/api/client", (req, res) => {
  const { name, firstname, birthdate, address, city, email, phone, comment } = req.body;
  db.query(
      "INSERT INTO clients(name_client, firstname_client, birthdate_client, address_client, city_client, email_client, phone_clients, comment_client) VALUES(?, ?, ?, ?, ?, ?, ?)", [name, firstname, birthdate, address, city, email, phone, comment],
      (err, result) => {
          if (err) {
              console.log(err);
...
app.get("/api/client/:id/achats", (req, res) => {
  db.query(
      "SELECT * from achats WHERE client_id_client=?", [req.params.id],
      (err, results) => {
          if (err) {
              console.log(err);
              res.status(500).send("Error retrieving data");
...
app.post("/api/client/:id/achats", (req, res) => {
  ...
  db.query(
      "INSERT INTO achats(type_achat, date_achat, quantity_achat, comment_achat, client_id_client) VALUES(?, ?, ?, ?, ?)", [type, date, quantity, comment, req.params.id],
      (err, result) => {
          if (err) {
              console.log(err);
              res.status(500).send("Error");
